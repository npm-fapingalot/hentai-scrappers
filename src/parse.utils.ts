...
export const isUndefined = <T>(val: T | undefined | null): val is null | undefined =>
  (val as T) === undefined || (val as T) === null;
...
/**
 * Renames the keys using the keymap. This Function edits the original value
 * @param obj The object being edited
 * @param remap The remap dictionary { [old]: new }
 */
export const renameKeys = (obj: any, remap: { [oldKey: string]: string; }) => {
  const copy = { ...obj };
  for (const key in remap) { if (copy[key] !== undefined) { copy[remap[key]] = copy[key]; delete copy[key]; } }
  return copy;
};
